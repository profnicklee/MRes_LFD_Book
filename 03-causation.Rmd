...
RR_points <- data.frame(
  ...
  label_x   = c(1.3, 0.2, 1.0, 4.0),
  label_y   = c(3.3, 1.55, -0.6, 2.6),
  hjust     = c(0.5, 1, 0.5, 0.5)
)
...
ggplot(RR_grid, aes(RR_smoke, RR_cancer, fill = Verdict)) +
  ...
  annotate("text", x = 9.5, y = 19, label = "Observed smoking-cancer RR = 9",
           hjust = 0, size = 3.5) +
  geom_point(data = RR_points, aes(RR_smoke, RR_cancer), inherit.aes = FALSE, size = 2) +
...
Below and to the left of both dashed lines, a candidate confounder simply can't do the job — it isn't strongly enough tied to *both* smoking and cancer to produce a ninefold relationship on its own. Above and to the right of both, it isn't mathematically ruled out — but now you have to actually name a real factor that's this strongly linked to becoming a smoker *and* this strongly linked to cancer, independently of smoking itself. Here's what happens when you actually go looking. The closest thing to Fisher's hypothesised factor that modern genetics has found — a gene variant linked to both nicotine dependence and lung cancer risk — clears neither bar by a wide margin. Radon gas meaningfully raises cancer risk but has nothing to do with who smokes. Asbestos exposure is the same shape. Heavy drinking is strongly tied to smoking but does essentially nothing to lung cancer risk on its own once you separate the two out. Four real, well-studied factors, marked on the plot above, four different ways of falling short — and not one of them gets remotely close to doing the job Fisher's argument needed. The point isn't that clearing both bars is impossible. It's that nobody, in over sixty years, has ever produced a real candidate that gets close.
