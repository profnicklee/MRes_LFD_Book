...
Nevertheless, this is just a scatterplot, so there is no real requirement to put any particular one of the variables on the x-axis — it's just a correlation, and Chapter \@ref(causation) is where we deal properly with why that's *not* the same thing as causation.
...
Of course, there are many other possible transforms. I touched on them earlier, in Chapter \@ref(intro), and I'll elaborate a little more on it below, for now let's rerun the plot using GDPpc_log
