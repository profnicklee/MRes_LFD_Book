...
Here, we will again use the simple three-variable set of simulated data from the demonstration of multiple regression in Chapter \@ref(assoc-rel), which represents rates of smoking, rates of cycling, and heart disease incidence.
...
You'll notice I use different functions to preview data through the book, and that's deliberate rather than sloppy. `head()`, like I've done here, is just for confirming the data loaded and what variables are there. `describe()` is for a fuller statistical snapshot of a data set I haven't looked at yet — including whether anything's missing — which is why I used it in the last chapter. And a plain `summary()`, like the one below, is for a quick look at a single variable I'm about to use. Three different jobs, so three different functions.
